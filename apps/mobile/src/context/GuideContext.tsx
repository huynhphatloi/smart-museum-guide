...
import { ActiveExhibitResponse } from '../api/types';
import { env } from '../config/env';
import { RegisteredBeacon, findZoneName } from '../features/ble/beacon-registry';
import { BeaconScanner, DEFAULT_SCANNER_CONFIG } from '../features/ble/scanner';
import { RealBleSignalSource } from '../features/ble/sources/real-ble-source';
import {
  SimulatedBleSignalSource,
  simulatedBeaconsFromRegistry,
} from '../features/ble/sources/simulated-ble-source';
import { DetectionSnapshot, ZoneConfirmedEvent } from '../features/ble/types';
...
interface GuideContextValue {
  ...
  updateCalibration: (patch: { dwellTimeMs?: number; hysteresisDb?: number }) => void;
  calibration: { dwellTimeMs: number; hysteresisDb: number; scanWindowMs: number };
...
export function GuideProvider({ children }: { children: React.ReactNode }) {
  ...
  const startScanning = useCallback(async () => {
    ...
    const config = {
      ...
      scanWindowMs: calibration.scanWindowMs,
      dwellTimeMs: calibration.dwellTimeMs,
      hysteresisDb: calibration.hysteresisDb,
    ...
    await scanner.start();
    setScanning(true);
  }, [registry, calibration, handleZoneConfirmed]);
  ...
  const updateCalibration = useCallback(
    (patch: { dwellTimeMs?: number; hysteresisDb?: number }) => {
      setCalibration((previous) => ({ ...previous, ...patch }));
      scannerRef.current?.updateDetectorConfig(patch);
    },
    [],
  );
  ...
  const value = useMemo<GuideContextValue>(
    () => ({
      ...
      registryError,
      reloadRegistry,
      isSimulation: env.bleSimulation,
      simulator,
      scanning,
      bleError,
      snapshot,
